...
void tchunk_generator::generate_chunks(tchunk_strategy_type strategy, uint32_t num, uint32_t image_width, uint32_t image_height, std::vector<tchunk>& out_chunks)
{
  uint32_t snum = (uint32_t)sqrt(num);
  switch (strategy)
  {
  case tchunk_strategy_type::vertical_stripes:
    return generate_rectangles(num, 1, image_width, image_height, out_chunks);
    
  case tchunk_strategy_type::horizontal_stripes:
    return generate_rectangles(1, num, image_width, image_height, out_chunks);
    
  case tchunk_strategy_type::rectangles:
    if (snum * snum != num)
    {
...
void tchunk_generator::generate_rectangles(uint32_t num_x, uint32_t num_y, uint32_t image_width, uint32_t image_height, std::vector<tchunk>& out_chunks)
{
